...
export default function  View({ notifications }: DropdownNotificationProps) {
  ...
  >
      ...
      <button
        className="cursor-pointer rounded-full border px-2 hover:border-blue-500"
    ...
    </div><div className="p-2 flex h-auto flex-col overflow-y-auto">
        {notifications && notifications.length > 0 ? (
          notifications.map((notification: Notification) => (
            <div key={notification.id} className="relative flex cursor-pointer">
              <div className="mt-3 flex w-full justify-between">
                <div
                  className={`mb-1 mr-2 flex justify-start ${notification.pivot.read_at === null
