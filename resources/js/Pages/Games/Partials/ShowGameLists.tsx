...
export default function ShowGameLists ({ game, sport, playground, teams }: ShowGameProps) {
    ...
    return (
        <div className="bg-gray-100 flex flex-col max-w-full lg:flex-row justify-around">
            {/* DISPLAY HOME OWNER & HOME TEAM DATAS */}
            <div className="ml-14 mt-10 mr-14 sm:mx-auto">
                <div className="grid gap-y-20 sm:gap-y-16">
                    <div className="max-w-2xl">
                        <p className="text-3xl tracking-tight text-green-700 sm:text-4xl text-center">
                            Home Team: {teams[0].name}
                        </p>
                    </div>
                    <ul role="list" className="flex flex-col gap-y-12">
                        {homeOwner && (
                            <li key={homeOwner.id} className="max-w-xs sm:max-w-none">
                                <div className="flex items-center gap-x-6">
                                    <img className="h-16 w-16 rounded-full" src={homeOwner.profile_photo_url} alt="" />
                                    <div>
                                    ...
                                    {homeOwner.id !== userID && (
                                        <PrimaryButton
                                            className="bg-green-700 text-white py-2 px-4 rounded mt-2 sm:flex flex-col"
                                            onClick={() => {
                                                setIsModalOpen(true);
                        ...
                        {homeTeam.map((person) => (
                            <li key={person.id} className="max-w-xs sm:max-w-none">
                                <div className="flex items-center gap-x-6">
                                    <img className="h-16 w-16 rounded-full" src={person.profile_photo_url} alt="" />
                                    <div>
                                        <h3 className="text-base font-semibold leading-7 tracking-tight text-gray-900">
                                            {person.firstname}
                                        </h3>
            ...
            {/* GAME DATAS */}
            <div className="rounded-lg mt-10 mx-10 max-w-68 max-h-auto py-8 sm:mx-auto px-6 lg:px-8 border-solid border-2 border-sky-600">
                ...
                <DefinitionList>
                    ...
                    <DefinitionListItem
                        term="From / To"
                        description={`${formatTime(game.start_time)} / ${formatTime(game.end_time)}`}
                    />
                    <DefinitionListItem
                        term="Players"
                        description={`${totalLength + 1} / ${game.max_player}`}
                    />
                    <DefinitionListItem
                        term="Playground"
                    ...
                    <DefinitionListItem
                        term="Teams"
                        description={`${teams[0].name}${teams[1] && `VS ${teams[1].name}`}`}
                    />
                    ...
                    <div className="flex justify-start">
                        ...
                        {userID === game.user_id && (
                            <>
                                <PrimaryButton className='opacity-80 mt-6 bg-green-700 mr-2' >
                        ...
                        <ul role="list" className="flex flex-col gap-y-12">
                            {awayOwner && (
                                <li key={awayOwner.id} className="max-w-xs sm:max-w-none">
                                    <div className="flex items-center gap-x-6">
                                        <img className="h-16 w-16 rounded-full" src={awayOwner.profile_photo_url} alt="" />
                                        <div>
                                            <h3 className="text-base font-semibold leading-7 tracking-tight text-gray-900">
                                                {awayOwner.firstname}
                                            </h3>
                                        ...
                                        {awayOwner.id !== userID && (
                                            <PrimaryButton
                                                className="bg-red-700 text-white py-2 px-4 rounded mt-2 sm:flex flex-col"
                                                onClick={() => {
                                                    setIsModalOpen(true);
