...
export default function ShowGameLists({ game, sport, playground, teams }: ShowGameProps) {
    ...
    return (
        <div className="flex flex-row">
                ...
                <li key={teams[0].id} className="flex justify-between gap-x-6 py-5">
                    <div className="flex min-w-0 gap-x-4">
                        <div className="min-w-0 flex-auto">
                            <h3>Home Team: {teams[0].name}</h3>
                            <img className="h-12 w-12 flex-none rounded-full bg-gray-50" src={homeOwner.profile_photo_url} alt="" />
                            <p className="text-sm font-semibold leading-6 text-gray-900">Team owner: {homeOwner.firstname}</p>
                            <PrimaryButton className='opacity-80 mb-2 bg-sky-500' >
                                <Link href={`/teams/${teams[0].id}/members`}>
                                    Contact
                                </Link>
                            </PrimaryButton>
                        </div>
                ...
                <div className="mt-6 border-t border-gray-100">
                    <dl className="divide-y divide-gray-100">
                        ...
                        <div className="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
                            ...
                            <dd className="text-sm leading-6 text-gray-700 sm:col-span-2">{totalLength + 1} / {game.max_player}</dd>
                        </div>
                        <div className="px-4 py-2 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-0">
            ...
            {awayTeam && (
                <ul role="list" className="divide-y divide-gray-100">
                    <li key={teams[1].id} className="flex justify-between gap-x-6 py-5">
                        <div className="flex min-w-0 gap-x-4">
                            <div className="min-w-0 flex-auto">
                                <h3>Home Team: {teams[1].name}</h3>
                                <img className="h-12 w-12 flex-none rounded-full bg-gray-50" src={awayOwner?.profile_photo_url} alt="" />
                                <p className="text-sm font-semibold leading-6 text-gray-900">Team owner: {awayOwner?.firstname}</p>
                                <PrimaryButton className='opacity-80 mb-2 bg-sky-500' >
                                    <Link href={`/teams/${teams[1].id}/members`}>
                                        Contact
                                    </Link>
                                </PrimaryButton>
                            </div>
